...
Calibration diagnostics are generated by:
- `src/scoring/backtest_scoring_calibration.py`

Approach:
- Reconstruct month-level risk scores from contemporaneous health/risk signals.
- Measure forward churn realization in the next 3 months.
- Evaluate calibration by risk tier and score decile.

Outputs:
- `data/processed/scoring_backtest_calibration_by_tier.csv`
- `data/processed/scoring_backtest_calibration_by_decile.csv`
- `reports/scoring_backtest_calibration.md`
- `reports/scoring_backtest_summary.json`
...
19. release-readiness governance artifact discipline
...
- `src/profiling/build_data_profile.py` -> `reports/profiling_stats.json`, `reports/data_profiling_memo.md`
...
Release artifacts:
- `reports/release_manifest.json` (runtime metadata, coverage window, row counts, validation snapshot)
- `reports/release_checksums.csv` (SHA256 checksums for core artifacts)
