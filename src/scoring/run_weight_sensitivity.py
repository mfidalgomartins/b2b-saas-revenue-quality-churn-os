...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--perturbation", type=float, default=0.20, help="Fractional shock applied to each weight (default 0.20)."
    )
...
def rebuild_governance_components(scoring: pd.DataFrame) -> dict[str, pd.Series]:
    ...
    exposure = 0.70 * clip01(np.log1p(scoring["current_mrr"]) / np.log1p(max(p99_mrr, 1.0))) + 0.30 * clip01(
        scoring["concentration_weight"] / 0.01
    )
    return {
        ...
        "renewal_urgency": clip01(
            scoring["renewal_due_flag"] * scoring["renewal_risk_proxy"] + scoring["renewal_due_flag"] * 0.15
        ),
    }
...
def main() -> None:
    ...
    latest = (
        amrq.sort_values("month").groupby("customer_id", as_index=False).tail(1)[["customer_id", "renewal_risk_proxy"]]
    )
    ...
    logger.info(
        "weight sensitivity complete (n=%d, max flip=%.2f%% on %s)",
        summary["n_accounts"],
