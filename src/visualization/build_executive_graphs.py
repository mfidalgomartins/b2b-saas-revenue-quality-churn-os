...
PDF reports, and slide decks. All charts use a consistent premium style:
...
def apply_style(ax: plt.Axes, spines: bool = True) -> None:
    """Apply consistent premium style to an axes object."""
    ax.set_facecolor(BG)
...
def chart_mrr_arr_growth(base: Path, out: Path, dpi: int) -> None:
    ...
    for spine in ax2.spines.values():
        spine.set_visible(False)
    ax2.plot(monthly["month"], monthly["arr"], color=ACCENT_LIGHT, linewidth=1.4,
             linestyle="--", zorder=2, alpha=0.6, label="ARR (right axis)")
    ...
    ax.annotate(
        ...
        fontsize=11, color=ACCENT, fontweight="semibold",
    )
    ...
    ax.set_title("Monthly Recurring Revenue — 36-Month View", fontsize=14, color="#0c0d0e",
...
def chart_grr_nrr_trend(base: Path, out: Path, dpi: int) -> None:
    ...
    monthly_rq = rq.groupby("month").agg(
        total_mrr=("active_mrr", "sum"),
        expansion=("expansion_mrr", "sum"),
        contraction=("contraction_mrr", "sum"),
    ).reset_index().sort_values("month")

    # Identify churned MRR each month
    churned_merge = (
        m[m["churn_flag"] == 1][["customer_id", "month"]]
        .merge(rq[["customer_id", "month", "active_mrr"]], on=["customer_id", "month"], how="left")
    )
    churned_mrr = churned_merge.groupby("month")["active_mrr"].sum().reset_index().rename(
        columns={"active_mrr": "churned_mrr"}
    )

    df = monthly_rq.merge(churned_mrr, on="month", how="left").fillna(0)
    df["prev_mrr"] = df["total_mrr"].shift(1)
    df = df.dropna(subset=["prev_mrr"])

    # GRR = (prev_mrr - churned_mrr - contraction_mrr) / prev_mrr, capped at 1.0
    df["grr"] = ((df["prev_mrr"] - df["churned_mrr"] - df["contraction"]) / df["prev_mrr"]).clip(upper=1.0)
    # NRR = (prev_mrr - churned_mrr - contraction + expansion) / prev_mrr
    df["nrr"] = (df["prev_mrr"] - df["churned_mrr"] - df["contraction"] + df["expansion"]) / df["prev_mrr"]
    ...
    ax.annotate("100% benchmark", xy=(df["month"].iloc[1], 1.0),
    ...
    ax.fill_between(df["month"], df["grr_smooth"], 1.0, where=df["grr_smooth"] < 1.0,
                    alpha=0.06, color=NEG, zorder=2)
    ax.fill_between(df["month"], df["nrr_smooth"], 1.0, where=df["nrr_smooth"] > 1.0,
                    alpha=0.06, color=ACCENT, zorder=2)
    ...
    ax.set_title("Gross & Net Revenue Retention — Monthly", fontsize=14, color="#0c0d0e",
                 fontweight="semibold", pad=14)
    ax.set_xlabel("")
...
def chart_logo_revenue_churn(base: Path, out: Path, dpi: int) -> None:
    ...
    logo = m.groupby("month").agg(active=("active_flag", "sum"), churned=("churn_flag", "sum")).reset_index()
    logo["logo_churn_rate"] = logo["churned"] / logo["active"]
    ...
    total_mrr = rq.groupby("month")["active_mrr"].sum().reset_index().rename(
        columns={"active_mrr": "total_mrr"}
    )
    rev = total_mrr.merge(churned_mrr, on="month", how="left").fillna(0)
    rev["rev_churn_rate"] = rev["churned_mrr"] / rev["total_mrr"]

    df = logo.merge(rev[["month", "rev_churn_rate"]], on="month")
    ...
    ax.bar(df["month"], df["logo_churn_rate"], width=20, alpha=0.18, color=NEUTRAL, zorder=1, label="Logo churn (monthly)")
    ax.bar(df["month"], df["rev_churn_rate"], width=20, alpha=0.22, color=NEG, zorder=1, label="Revenue churn (monthly)")
    ax.plot(df["month"], df["logo_smooth"], color=NEUTRAL, linewidth=2.0, zorder=3)
    ax.plot(df["month"], df["rev_smooth"], color=NEG, linewidth=2.0, zorder=3)
    ...
    # End-point labels
    last = df.iloc[-1]
    ax.annotate(f"Logo {last.logo_smooth*100:.2f}%",
                xy=(last["month"], last["logo_smooth"]),
                xytext=(6, 0), textcoords="offset points",
                fontsize=9.5, color=NEUTRAL, va="center")
    ax.annotate(f"Revenue {last.rev_smooth*100:.2f}%",
                xy=(last["month"], last["rev_smooth"]),
                xytext=(6, -12), textcoords="offset points",
                fontsize=9.5, color=NEG, va="center")
    ...
    ax.set_title("Logo Churn vs Revenue Churn — Monthly Rate", fontsize=14, color="#0c0d0e",
                 fontweight="semibold", pad=14)
    ax.set_xlabel("")
...
def chart_revenue_quality_dist(base: Path, out: Path, dpi: int) -> None:
    ...
    for tier, color in zip(tier_order, tier_colors, strict=True):
        ...
        ax.hist(subset, bins=bins, color=color, alpha=0.75, edgecolor=BG, linewidth=0.4,
                label=f"{tier} ({len(subset):,})", zorder=2)
    ...
    ax.legend(frameon=False, fontsize=10, labelcolor=NEUTRAL, title="Quality tier",
              title_fontsize=9)
    ax.set_title("Revenue Quality Score — Account Distribution", fontsize=14, color="#0c0d0e",
                 fontweight="semibold", pad=14)
...
def chart_churn_risk_dist(base: Path, out: Path, dpi: int) -> None:
    ...
    ax.legend(frameon=False, fontsize=10, labelcolor=NEUTRAL, title="Churn risk tier",
              title_fontsize=9)
    ax.set_title("Churn Risk Score — Account Distribution", fontsize=14, color="#0c0d0e",
                 fontweight="semibold", pad=14)
...
def chart_discount_trend(base: Path, out: Path, dpi: int) -> None:
    ...
    p = np.poly1d(z)
    trend = p(x_num)
    ax.plot(monthly["month"], trend, color=NEUTRAL, linewidth=1.2, linestyle="--",
            alpha=0.6, label="Trend (linear fit)")
    ...
    ax.annotate(
        ...
        fontsize=11, color=NEG, fontweight="semibold",
    ...
    ax.legend(frameon=False, fontsize=10, labelcolor=NEUTRAL)
    ax.set_title("Discount-Reliant MRR Share — Monthly", fontsize=14, color="#0c0d0e",
                 fontweight="semibold", pad=14)
    ax.set_xlabel("")
...
def chart_at_risk_concentration(base: Path, out: Path, dpi: int) -> None:
    ...
    ax.plot(at_risk["pct_accounts"] * 100, at_risk["cumulative_pct_mrr"] * 100,
            color=ACCENT, linewidth=2.4, zorder=3, label="Concentration curve")
    ax.fill_between(at_risk["pct_accounts"] * 100, at_risk["cumulative_pct_mrr"] * 100,
                    alpha=0.07, color=ACCENT, zorder=2)

    # 45-degree equal distribution line
    ax.plot([0, 100], [0, 100], color=NEUTRAL, linewidth=1.0, linestyle="--",
            alpha=0.5, label="Equal distribution")
    ...
    ax.annotate(f"Top 20% accounts\n= {share_at_20:.0f}% of at-risk MRR",
                xy=(20, share_at_20), xytext=(10, -30), textcoords="offset points",
                fontsize=10, color=NEG,
                arrowprops={"arrowstyle": "-", "color": NEUTRAL, "alpha": 0.5})
    ...
    ax.legend(frameon=False, fontsize=10, labelcolor=NEUTRAL)
    ax.set_title(
        f"At-Risk MRR Concentration  ·  {len(at_risk):,} accounts  ·  "
        f"${total_at_risk/1e6:.1f}M at risk",
        fontsize=13, color="#0c0d0e", fontweight="semibold", pad=14,
    )
...
def chart_scenario_trajectories(base: Path, out: Path, dpi: int) -> None:
    ...
    # Historical line
    ax.plot(historical["month"], historical["mrr"], color=NEUTRAL, linewidth=2.0,
            zorder=3, label="Actuals")
    ...
    for scenario in displayed:
        ...
        ax.plot(x, y, color=color, linewidth=1.8 if scenario != "base_case" else 2.4,
    ...
    ax.text(last_actual_date, ax.get_ylim()[0] * 1.02 if ax.get_ylim()[0] > 0 else
            (ax.get_ylim()[1] - ax.get_ylim()[0]) * 0.02 + ax.get_ylim()[0],
            "  Forecast >>", fontsize=9, color=NEUTRAL)
    ...
    ax.legend(frameon=False, fontsize=10, labelcolor=NEUTRAL, loc="upper left",
              ncol=2)
    ax.set_title("MRR Scenario Trajectories — Actuals + 6-Month Forecast",
                 fontsize=14, color="#0c0d0e", fontweight="semibold", pad=14)
    ax.set_xlabel("")
...
def chart_cohort_heatmap(base: Path, out: Path, dpi: int) -> None:
    ...
    max_months = 13
    pivot = (
        cohort_agg[cohort_agg["month_number"] <= max_months]
        .pivot(index="cohort_month", columns="month_number", values="nrr")
    )
    ...
    ax.set_title("Cohort Net Revenue Retention — Month 0 to 12",
                 fontsize=14, color="#0c0d0e", fontweight="semibold", pad=14)
...
def chart_governance_priority_accounts(base: Path, out: Path, dpi: int) -> None:
    ...
    for bar, score, tier in zip(
        ...
        ax.text(
            ...
            f"{score:.0f}  {tier}",
            va="center", ha="left", fontsize=9, color=PALETTE[tier],
        )
    ...
    legend_elements = [
        ...
        for t in ["Critical", "High", "Moderate"] if t in top["governance_priority_tier"].values
    ]
    ax.legend(handles=legend_elements, frameon=False, fontsize=10, labelcolor=NEUTRAL,
              title="Governance tier", title_fontsize=9)

    ax.xaxis.set_major_formatter(mticker.FuncFormatter(fmt_currency))
    ax.set_title("Top 20 Accounts by Governance Priority — Current MRR & Risk Score",
                 fontsize=13, color="#0c0d0e", fontweight="semibold", pad=14)
    ax.set_xlabel("Current MRR", fontsize=10, color=NEUTRAL)
    ax.set_ylabel("")
    ax.invert_xaxis() if False else None  # bars read left-to-right
    fig.tight_layout()
